...
if __name__ == '__main__':
    ...
    #仁寶大樓
    #spatial = "spatialFilter=nearby(25.079454361644743, 121.56844333062183,30)"
    ...
    # response = request('get', 'https://ptx.transportdata.tw/MOTC/v2/Bus/Stop/City/Taipei?$top=2&$format=JSON', headers= a.get_auth_header())
    response = request('get', url, headers= a.get_auth_header())
    # pprint(response.content)
    x = json.loads(response.content)

    for item in x:
        # lineout = "StationID:%s | StopID:%s | StopName:%s | RouteID:%s" % \
        # (item['StationID'], item['Stops'][0]['StopID'], \
        # item['Stops'][0]['StopName']['Zh_tw'], item['Stops'][0]['RouteName']['En'])
        ...
        print(lineout)
        temparr = []
        watchlist = ["936","937","945","946","946sub"]
        for stop in item['Stops']:
            if stop['RouteName']['En'] in watchlist:
                ...
                print(temp)
        # print(temparr)
        # lineout += ", ".join(temparr)

        # Zh_tw
        # En
        #  item['Stops'][0]['StopName']['Zh_tw'], item['Stops'][0]['RouteID'])
        # print(lineout)
        # print(item)
        # print("======")
        # print(item['StationName']['Zh_tw'])
